#!/usr/bin/env python3
"""
generate_thread_report.py

Scans benchmark_logs/ for *.stat-thpool.txt and *.stat-thr.txt files, parses thread metrics,
and generates separate interactive HTML reports for each type.

Usage:
  python3 generate_thread_report.py [base_dir]

Defaults:
  base_dir = "benchmark_logs"

Output:
  thread_pool_report.html - Thread pool metrics
  threads_report.html - Threads metrics
"""
...
def parse_thread_file(path: str, file_type: str, store_original_path: bool = True) -> Optional[Dict]:
    """Parse a single thread status file.

    Args:
        path: Path to the file
        file_type: Either 'thpool' or 'thr'
        store_original_path: Whether to store the original file path in the result
    """
    parts = Path(path).parts
    original_path = path if store_original_path else None

    # Expected: benchmark_logs/{db_type}/{version}/run{N}_Tier{M}G_RW_{T}th.stat-{type}.txt
    if len(parts) < 4:
    ...
    # Read file - format: timestamp<tab>variable_name<tab>value
    try:
    ...
    for line in lines:
        ...
        parts = line.split('\t')
        if len(parts) == 3:
            timestamp, metric_name, value = parts

            # Check if value is in compound format: avg: X, min: Y, max: Z, dev: W, cnt: V
            if value.startswith('avg:') or ', avg:' in value:
                # Parse compound metrics
                import re
                compound_parts = {}
                for match in re.finditer(r'(\w+):\s*([\d.]+)', value):
                    sub_metric = match.group(1)
                    sub_value = match.group(2)
                    compound_key = f"{metric_name}.{sub_metric}"
                    data_by_timestamp[timestamp][compound_key] = sub_value
                    all_metrics.add(compound_key)
            else:
                # Simple metric
    ...
    # Convert to list of rows with timestamp column
    metrics_list = ['timestamp'] + sorted(all_metrics)
    data_rows = []

    for ts in sorted(data_by_timestamp.keys(), key=float):
        row = {'timestamp': ts}
        for metric in all_metrics:
            row[metric] = data_by_timestamp[ts].get(metric, '')
...
def generate_metric_options_html(simple_metrics, compound_metrics, compound_bases_sorted):
    """Generate HTML for metric options with grouping for compound metrics."""
    html_parts = []
    counter = 0
    ...
    for metric in simple_metrics:
        html_parts.append(
            f'<div class="metric-option" data-metric="{metric}">'
            f'<input type="checkbox" value="{metric}" id="metric_{counter}" onchange="updateSelectedMetrics()">'
            f'<label for="metric_{counter}">{metric}</label></div>'
        )
        counter += 1

    # Compound metrics (grouped)
    for base_name in compound_bases_sorted:
        sub_metrics = compound_metrics[base_name]
        html_parts.append(f'<div class="metric-group" data-base="{base_name}">')
        html_parts.append(
            f'<div class="metric-group-header" onclick="toggleMetricGroup(\'{base_name}\')">'
            f'{base_name} <span class="metric-group-toggle">▼</span></div>'
        )
        html_parts.append(f'<div class="metric-submetrics" id="group_{base_name.replace(".", "_")}">')

        for sub_metric in sub_metrics:
            full_metric = f"{base_name}.{sub_metric}"
            html_parts.append(
                f'<div class="metric-option" data-metric="{full_metric}">'
                f'<input type="checkbox" value="{full_metric}" id="metric_{counter}" onchange="updateSelectedMetrics()">'
                f'<label for="metric_{counter}">{sub_metric}</label></div>'
            )
            counter += 1
...
def generate_html_report(parsed_files: List[Dict], output_file: str, title: str, data_dir_name: str):
    """Generate HTML report for thread metrics."""
    ...
    for pf in parsed_files:
        all_metrics.update([m for m in pf['metrics'] if m != 'timestamp'])
        all_servers.add(pf['server'])
        all_runs.add(pf['run'])
        all_threads.add(pf['threads'])
    ...
    # Group compound metrics
    simple_metrics = []
    compound_metrics = defaultdict(list)

    for metric in all_metrics:
        if '.' in metric:
            # Compound metric: base_name.sub_metric
            base_name, sub_metric = metric.rsplit('.', 1)
            compound_metrics[base_name].append(sub_metric)
        else:
            simple_metrics.append(metric)

    # Sort everything
    simple_metrics.sort()
    for base_name in compound_metrics:
        compound_metrics[base_name].sort()
    compound_bases_sorted = sorted(compound_metrics.keys())

    # Build flat list for backwards compatibility
    metrics_sorted = simple_metrics + [f"{base}.{sub}" for base in compound_bases_sorted for sub in compound_metrics[base]]
    ...
    for pf in parsed_files:
        ...
        safe_base = f"{sanitize_filename(server)}_run{run}_{threads}th"
        json_path = data_dir / f"{safe_base}.json"

        # Copy original text file to data directory if available
        txt_filename = None
        if pf.get('original_path'):
            import shutil
            original_ext = Path(pf['original_path']).suffix  # .txt
            txt_filename = f"{safe_base}{original_ext}"
            txt_path = data_dir / txt_filename
            try:
                shutil.copy2(pf['original_path'], txt_path)
                print(f"  Copied: {txt_path} ({os.path.getsize(txt_path) / 1024:.1f} KB)")
            except Exception as e:
                print(f"  Warning: Could not copy {pf['original_path']}: {e}")
                txt_filename = None
        ...
        data_manifest[key] = {
            'json': f"{data_dir_name}/{safe_base}.json",
            'txt': f"{data_dir_name}/{txt_filename}" if txt_filename else None
        }
        print(f"  Written: {json_path} ({os.path.getsize(json_path) / 1024:.1f} KB)")

    # Generate HTML (reusing the same structure as innodb_metrics_report.py)
...
<head>
    ...
    <style>
        ...
        .metric-submetrics .metric-option {{
            padding-left: 24px;
            font-size: 13px;
        }}
        .selected-metrics {{
...
<body>
    <div class="container">
        <h1>{title}</h1>
        <p class="subtitle">Interactive comparison of thread metrics across servers and runs</p>
                ...
                <div class="metric-multiselect">
                    ...
                    <div id="metricDropdown" class="metric-dropdown">
                        ...
                        <div id="metricOptions">
                            {generate_metric_options_html(simple_metrics, compound_metrics, compound_bases_sorted)}
                        </div>
        ...
        <div class="download-section" id="downloadSection">
            <h3>Download Original Data Files</h3>
            <p style="color: #666; font-size: 13px; margin-bottom: 15px;">
                Download the raw text files used to generate this report for the selected combinations.
            </p>
    ...
    <script>
        ...
        function toggleMetricGroup(baseName) {{
            const groupId = 'group_' + baseName.replace(/\\./g, '_');
            const group = document.getElementById(groupId);
            const header = group.previousElementSibling;
            const toggle = header.querySelector('.metric-group-toggle');
        ...
        function filterMetrics(searchTerm) {{
            ...
            groups.forEach(group => {{
                const visibleChildren = group.querySelectorAll('.metric-option:not(.hidden)');
                if (visibleChildren.length > 0) {{
                    group.style.display = 'block';
                    // Expand group if search is active
                    if (searchTerm) {{
                        const submetrics = group.querySelector('.metric-submetrics');
                        const toggle = group.querySelector('.metric-group-toggle');
                        if (submetrics && toggle) {{
        ...
        async function loadData(key) {{
            ...
            const manifestEntry = DATA_MANIFEST[key];
            if (!manifestEntry) {{
                return null;
            }}

            const filePath = typeof manifestEntry === 'string' ? manifestEntry : manifestEntry.json;
        ...
        function updateDownloadSection(validCombinations) {{
            ...
            validCombinations.forEach(combo => {{
                const key = combo.key;
                const manifestEntry = DATA_MANIFEST[key];
                const txtPath = typeof manifestEntry === 'string' ? null : manifestEntry.txt;

                if (txtPath) {{
                    const btn = document.createElement('a');
                    btn.className = 'download-btn';
                    btn.href = txtPath;
                    btn.download = txtPath.split('/').pop();
                    btn.innerHTML = `
                        <span class="download-icon">⬇</span>
                        <span>${{combo.server}} Run ${{combo.run}} (${{combo.threads}}th)</span>
                    `;
        ...
        async function generateData() {{
            ...
            for (let i = 0; i < maxRows; i++) {{
                ...
                for (const combo of validCombinations) {{
                    ...
                    for (const metric of selectedMetricsList) {{
                        if (row && row[metric] !== undefined && row[metric] !== '') {{
            ...
            // Update download section with available files
            updateDownloadSection(validCombinations);
                ...
                options: {{
                        ...
                        tooltip: {{
                            callbacks: {{
                                title: function(context) {{
                                    ...
                                    return `Time: ${{minutes}}m ${{secs}}s (row ${{seconds}})`;
                                }},
                    ...
                    scales: {{
                        x: {{
                            ...
                            title: {{
                                display: true,
                                text: 'Sample Index (approximately 1 per second)'
                            }},
                            ticks: {{
...
def main():
    ...
    # Find all thread status files
    base_path = Path(base_dir)
    if not base_path.exists():
        print(f"Error: Directory '{base_dir}' does not exist")
        sys.exit(1)

    thpool_files = list(base_path.rglob("*.stat-thpool.txt"))
    thr_files = list(base_path.rglob("*.stat-thr.txt"))

    print(f"Found {len(thpool_files)} threadpool file(s)")
    print(f"Found {len(thr_files)} threads file(s)")

    # Parse thread pool files
    if len(thpool_files) > 0:
        parsed_thpool = []
        for filepath in thpool_files:
            result = parse_thread_file(str(filepath), 'thpool')
            if result:
                parsed_thpool.append(result)

        if len(parsed_thpool) > 0:
            generate_html_report(
                parsed_thpool,
                "thread_pool_report.html",
                "Thread Pool Status Analysis",
                "thread_pool_data"
            )
        else:
            print("Warning: No valid threadpool data could be parsed")
    else:
        print("No threadpool files found")

    # Parse threads files
    if len(thr_files) > 0:
        parsed_thr = []
        for filepath in thr_files:
            result = parse_thread_file(str(filepath), 'thr')
            if result:
                parsed_thr.append(result)

        if len(parsed_thr) > 0:
            generate_html_report(
                parsed_thr,
                "threads_report.html",
                "Threads Status Analysis",
                "threads_data"
            )
        else:
            print("Warning: No valid threads data could be parsed")
    else:
        print("No threads files found")
