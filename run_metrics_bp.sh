...
# Buffer pool instances to test for Percona Server (MySQL will use default calculation)
# For Percona Server, test with different instance counts to analyze flush contention
BP_INSTANCES_TO_TEST=(4 8)
...
IS_READ_ONLY="$3"
ENABLE_BINLOG="$4"
BP_INSTANCES_OVERRIDE="$5"  # Optional: Override buffer pool instances for this run
...
# Add buffer pool instance suffix if specified
if [[ -n "$BP_INSTANCES_OVERRIDE" ]]; then
    BENCH_DIR="${BENCH_DIR}_bp${BP_INSTANCES_OVERRIDE}"
fi
...
generate_config() {
    ...
    # Use override if provided, otherwise calculate based on pool size
    if [[ -n "$BP_INSTANCES_OVERRIDE" ]]; then
        INSTANCES="$BP_INSTANCES_OVERRIDE"
        echo ">>> Using buffer pool instances override: $INSTANCES"
    else
        INSTANCES=$(( SIZE / 5 ))
...
start_offcpu_profiling() {
    local PREFIX=$1
    local OUT="${PREFIX}.offcpu.txt"
    local DELAY=500

    echo "off-CPU profiling -> ${OUT} (will start after ${DELAY}s)"

    (
        # Wait 500 seconds before starting off-CPU profiling
        echo "Waiting ${DELAY} seconds before starting off-CPU profiling..." > "$OUT"
        sleep $DELAY

        # Find mysqld PID
        local MYSQLD_PID=$(pgrep -x mysqld)

        if [ -z "$MYSQLD_PID" ]; then
            echo "ERROR: Could not find mysqld process" >> "$OUT"
            exit 1
        fi

        echo "" >> "$OUT"
        echo "Starting off-CPU profiling at $(date)" >> "$OUT"
        echo "Target PID: $MYSQLD_PID (mysqld)" >> "$OUT"
        echo "Duration: 30 seconds" >> "$OUT"
        echo "================================================" >> "$OUT"
        echo "" >> "$OUT"

        # Run offcputime-bpfcc with folded output (-f) and delta times (-d)
        # -p: target PID, 30: duration in seconds
        sudo offcputime-bpfcc -df -p "$MYSQLD_PID" 30 >> "$OUT" 2>&1

        echo "" >> "$OUT"
        echo "Off-CPU profiling completed at $(date)" >> "$OUT"
    ) &
    echo $! > /tmp/offcpu.pid
}
...
start_metrics() {
    ...
    start_offcpu_profiling "$PREFIX"
}

stop_metrics() {
    kill $(cat /tmp/iostat.pid) $(cat /tmp/vmstat.pid) $(cat /tmp/mpstat.pid) $(cat /tmp/dstat.pid) $(cat /tmp/innodb.pid) $(cat /tmp/gdb.pid 2>/dev/null) $(cat /tmp/offcpu.pid 2>/dev/null) 2>/dev/null
...
for SIZE in "${POOL_SIZES[@]}"; do
  ...
  if [[ -n "$BP_INSTANCES_OVERRIDE" ]]; then
      TIER_DATADIR="${TIER_DATADIR}_bp${BP_INSTANCES_OVERRIDE}"
  fi
